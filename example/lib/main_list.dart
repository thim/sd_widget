...
class _MyHomePageState extends State<MyHomePage> {
  ...
  final JsonViewRegistry registry = defaultWidget();
  late final JsonViewDataBuilder viewData;

  _MyHomePageState() {
    viewData = JsonViewDataBuilder(registry);
  ...
  void _incrementCounter() {
    setState(() {
      _counter++;
      registry.addData({'counter': _counter});
    });
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: ListView.builder(
        itemBuilder: (BuildContext context, int index) => viewData.build(context, index: index),
